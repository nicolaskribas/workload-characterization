...
```
Percentual de requisições usando *POST*.

```{r}
nrow(df[df$method == "HEAD",])/nrow(df) * 100
```
Percentual de requisições usando *HEAD*.
...
* 48,2% das requisições serão feitas para objetos de tamanho pequeno (0 a 8000 bits)
* 15,1% das requisições serão feitas para objetos de tamanho médio (8000 a 28000 bits)
* 34,2% das requisições serão feitas para objetos de tamanho grande (28000 a 45000 bits)
* 2,6% das requisições serão feitas para objetos de tamanho gigante (45000 a 125000 bits)
